...
func addScore(datastuffs *sql.DB, name string,  password string, score string) error{
        var userinfo string
        var MD5Password string
        MD5Password = getMD5(name+""+password)
        userinfo += " where password = '"+MD5Password+"';"
        fmt.Println(userinfo)
        err1 := updateDBrow(datastuffs,"score", score, "users" , userinfo)
        return err1
}
...
func updateDBrow(datastuffs *sql.DB, attribute string, invalue string, table string, condtion string) error{
...
func validateUser(datastuffs *sql.DB, Player player) bool {
	...
	if (userExists(datastuffs, Player.getPlayerLogin())) {
		fmt.Print("validateing user MD5: ")
...
func submitScore(w http.ResponseWriter, r *http.Request) {
	if (addScore(databaseConnect(), r.URL.Query()["user"][0], r.URL.Query()["pass"][0], r.URL.Query()["score"][0])==nil){
		json.NewEncoder(w).Encode("adding score")
	}
}
